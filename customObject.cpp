    ...
    const int bitflags)
{
    ...
    for (int i = 0; i < gripAppData.length(); i++)
    {
        ...
        case 6:
            ...
            updateDimensions(this, AcGePoint3d(-h / 2, -R, 0), AcGePoint3d(h / 2, -R, 0));
            return Acad::eOk;
        }
    }
    return Acad::eOk;
...
void  customObject::MyGripHotGripStretchpoints(AcDbGripData* pGripData, const  AcDbObjectId& entId, double  dimScale, AcDbDimDataPtrArray& dimDataArr)
{
    ...
    es = pDimData->setDimInvisible(false);
    es = pDimData->setDimResultantLength(true);
    es = pDimData->setDimHideIfValueIsZero(true);
    es = pDimData->setDimValueFunc(setDimValueForH);
